...
**loom does not run yet.**

loom is written in twill, in `.tw` files, and it uses `mode systems`, the
...
there is no binary, no `loom` command, and every code block below describes
behaviour loom is written to have rather than behaviour it has.

Writing a real framework against the subset is how you find out what the subset
is missing. What it is missing is written down in
[`docs/needs.md`](docs/needs.md): one entry per feature, naming the file and the
function that needs it and what loom does in the meantime. That list is the
useful output of this repository today.
